...
//Return x + y*nx
inline int SIMPLE_PAINTER::computeCanvasIndex( int x, int y, int nx, int ny )const
{
    return x+y*nx;
}
...
{
    //color = vector3( x, y, 0.0 );
	//int index;
	//index=computeCanvasIndex(x,y,x,y);
	//color = mCanvas[index];
...
{
	if (x>mNX || y>mNY || x<0 || y<0) return;
    if (color.r>1 || color.r<0 || color.g>1 || color.g<0 || color.b>1 || color.b<0) return;
	int index=y*mNX+x;
