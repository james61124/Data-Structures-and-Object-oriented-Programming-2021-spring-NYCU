...
namespace ns_opengl
{
	...
	void dTEXTURE::drawTexture( ) const
	{
		...
		int i;
		for (i=0;i<4; i++) {
			glTexCoord2f(data_cube[5*i], data_cube[5*i+1]); 
			glVertex3f(data_cube[5*i+2], 
                data_cube[5*i+3], data_cube[5*i+4]);
		}
		glEnd();
	}
	...
	void dTEXTURE::drawTexture(float x, float y, float z, float sx, float sy, float sz) const
	{
		...
		for (i=0;i<4; i++) {
			glTexCoord2f( data_cube[5*i], 
                data_cube[5*i+1] ); 
			glVertex3f( data_cube[5*i+2]*sx+x, 
