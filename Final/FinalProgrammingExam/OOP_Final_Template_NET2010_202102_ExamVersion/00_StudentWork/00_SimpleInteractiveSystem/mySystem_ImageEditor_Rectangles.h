...
#ifndef __MY_IMAGE_SYSTEM_RECTANGLES_H_
#define __MY_IMAGE_SYSTEM_RECTANGLES_H_
#include "../headers.h"
#include <string>
...
public:
    ...
    int cur_depth;
        ...
        int h;
        float sx; float sy;
    IMAGE_NODE_REC *children[4];
    float draw_x, draw_y;
    IMAGE_NODE_REC *next;
    IMAGE_NODE_REC *prev;
};

class IMAGE_SYSTEM_RECTANGLES : public BASE_SYSTEM {
public:
...
protected:
    ...
    int computeNumOfNodes(IMAGE_NODE_REC *node, int &numNodes) const;

    void replay_forward();
    void replay_backward();

    void reset(IMAGE_NODE_REC *node);
    void computeNodeColor(IMAGE_NODE_REC *node);
    void collapseImageNode(IMAGE_NODE_REC *node);
    void collapseImageNode(IMAGE_NODE_REC *parent_node, IMAGE_NODE_REC *node, double x, double y);
    void exploreImageNode(IMAGE_NODE_REC *node, double x, double y);
    //
    bool exploreImageDownwards( IMAGE_NODE_REC *node, double x, double y, bool flg_auto, int code );
    ...
    void draw_Disks_Dynamic( const IMAGE_NODE_REC *node) const;

    void draw_Disks_Dynamic( ) const;

    void draw_Rectangles_Dynamic( const IMAGE_NODE_REC *node, bool flg_draw_all) const;
    ...
    void drawDisk_TriangleFan( const IMAGE_NODE_REC *node ) const;
...
public:
    IMAGE_SYSTEM_RECTANGLES();
protected:
    ns_opengl::Image *m_Image;
    int m_Depth;

    IMAGE_NODE_REC *m_CurrentData;

    IMAGE_NODE_REC *m_ImageHierarchy;

   

    IMAGE_NODE_REC *m_CurrentNode_Replay;
    IMAGE_NODE_REC *m_Replay_Head;
    IMAGE_NODE_REC *m_Replay_Tail;
    //
    bool mFlgDrawnInRainbow;

    int m_Width_ImageBlocks;
    int m_Height_ImageBlocks;
     IMAGE_NODE_REC *m_ImageBlockPool;
     IMAGE_NODE_REC *m_ImageBlockHead;
     IMAGE_NODE_REC *m_ImageBlockTail;
};
