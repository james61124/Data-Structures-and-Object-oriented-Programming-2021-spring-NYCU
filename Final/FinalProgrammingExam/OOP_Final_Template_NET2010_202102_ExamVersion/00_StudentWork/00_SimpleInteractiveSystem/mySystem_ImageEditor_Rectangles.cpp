...
IMAGE_SYSTEM_RECTANGLES::IMAGE_SYSTEM_RECTANGLES()
{
    ...
    m_ImageHierarchy = new IMAGE_NODE_REC(
        cur_depth, 
        start_x,
        start_y,
        w,
    ...
    m_ImageBlockHead = 0;
    m_ImageBlockTail = 0;
    m_ImageBlockPool = new IMAGE_NODE_REC[W_MAX_IMAGE_BLOCKS*H_MAX_IMAGE_BLOCKS];
//
    m_CurrentData = m_ImageHierarchy;
}

void IMAGE_SYSTEM_RECTANGLES::reset(IMAGE_NODE_REC *node)
{
...
void IMAGE_SYSTEM_RECTANGLES::reset()
{
    m_CurrentNode_Replay = 0;
    m_Replay_Head = 0;
    m_Replay_Tail = 0;
...
void IMAGE_SYSTEM_RECTANGLES::replay_backward()
{
...
void IMAGE_SYSTEM_RECTANGLES::replay_forward()
{
    if ( m_Replay_Head == 0) return;
...
bool IMAGE_SYSTEM_RECTANGLES::handleKeyPressedEvent( unsigned char key )
{
    switch(key) {
...
void IMAGE_SYSTEM_RECTANGLES::createSpiralBlocksIn_Clockwise()
{
    ...
    int dir = 0;
    while (true) {
        IMAGE_NODE_REC *node = &m_ImageBlockPool[blockIndex++];
        node->resetChildren();
...
void IMAGE_SYSTEM_RECTANGLES::createSpiralBlocksOut_CounterClockwise()
{
    createSpiralBlocksIn_Clockwise();
    vector<IMAGE_NODE_REC*> ptrArr;
    for ( IMAGE_NODE_REC* p = m_ImageBlockHead; p; p = p->children[0]) {
...
void IMAGE_SYSTEM_RECTANGLES::createSnakeBlocks_BottomUp( ) {
    m_Width_ImageBlocks = 32;
    m_Height_ImageBlocks = 32;
...
void IMAGE_SYSTEM_RECTANGLES::createSnakeBlocks_TopDown( ) {
    createSnakeBlocks_BottomUp( );
    vector<IMAGE_NODE_REC*> ptrArr;
    for ( IMAGE_NODE_REC* p = m_ImageBlockHead; p; p = p->children[0]) {
        p->flg_explored = false;
...
void IMAGE_SYSTEM_RECTANGLES::computeNodeColor(IMAGE_NODE_REC *node)
{
    float r, g, b;
    computeAverageColor(
        node->start_x,
...
void IMAGE_SYSTEM_RECTANGLES::exploreImageNode(IMAGE_NODE_REC *node, double x, double y)
{
    if (!node) return;
    ...
    if (node->flg_explored) {
        ...
        int wImage, hImage;
        m_Image->getInfo(wImage, hImage);
        IMAGE_NODE_REC *children[4] = {0,0,0,0};
        float radius = node->w/2.0;
        children[0] = new IMAGE_NODE_REC(
            node->cur_depth+1, 
            node->start_x, 
            node->start_y,
            //node->w/2, node->h/2, 
...
bool IMAGE_SYSTEM_RECTANGLES::handlePassiveMouseEvent( double x, double y )
{
...
void IMAGE_SYSTEM_RECTANGLES::collapseImageNode(IMAGE_NODE_REC *node)
{
    if (node ==0) return;
    node->flg_explored = false;
    for (int i =0; i <4; ++i) {
...
void IMAGE_SYSTEM_RECTANGLES::collapseImageNode(
    IMAGE_NODE_REC *parent_node
    , IMAGE_NODE_REC *node
    , double x
    , double y
    )
...
bool IMAGE_SYSTEM_RECTANGLES::exploreImageDownwards( 
    IMAGE_NODE_REC *node, double x, double y, bool flg_auto, int code )
{
    if (!node) return true;
    if (node->flg_explored) {
        int ccode = 1;
...
bool IMAGE_SYSTEM_RECTANGLES::exploreImageDownwards( double x, double y, bool flg_auto )
{
...
bool IMAGE_SYSTEM_RECTANGLES::exploreImageUpwards( double x, double y, bool flg_auto )
{
...
bool IMAGE_SYSTEM_RECTANGLES::exploreImageLeftToRight( double x, double y, bool flg_auto )
{
    exploreImageDownwards(m_ImageHierarchy, x, y, flg_auto, 0x0a);
    return true;
}

    bool IMAGE_SYSTEM_RECTANGLES::exploreImageRightToLeft( double x, double y, bool flg_auto )
    {
        exploreImageDownwards(m_ImageHierarchy, x, y, flg_auto, 0x05);
    return true;
    }

bool IMAGE_SYSTEM_RECTANGLES::handleMotionMouseEvent( double x, double y )
{
...
int IMAGE_SYSTEM_RECTANGLES::computeNumOfNodes(IMAGE_NODE_REC *node, int &numNodes) const
{
...
int IMAGE_SYSTEM_RECTANGLES::computeNumOfNodes() const {
    int numNodes = 0;
    //m_CurrentData = m_ImageHierarchy;
