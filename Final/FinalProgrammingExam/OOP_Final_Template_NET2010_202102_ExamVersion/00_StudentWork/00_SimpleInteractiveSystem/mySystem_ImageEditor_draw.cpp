...
namespace {
    ...
    void drawRectangle(double x, double y, double r)
    {
        int numSamples = 9;
        r = r*0.9;
...
void IMAGE_SYSTEM::draw_Rectangles_Dynamic( 
    const IMAGE_NODE *node, bool flg_draw_all
    ) const{
    if (node==0) return;
    //if (node->flg_explored == false) return;
...
void IMAGE_SYSTEM::draw_Rectangles_Dynamic( ) const
{
    bool flg_draw_all = (m_CurrentData == m_ImageBlockHead);
    draw_Rectangles_Dynamic(m_CurrentData, flg_draw_all);
}

void IMAGE_SYSTEM::draw_Rectangles_Dynamic_InRainbow(
...
void IMAGE_SYSTEM::draw_Rectangles_Dynamic_InRainbow() const
{
...
void IMAGE_SYSTEM::draw_in_rainbow( ) const
{
    initColorTable();
